  ...
  AuthBloc({required this.authRepository}) : super(UnAuthenticated()) {
    ...
    on<SignInRequested>((event, emit) async {
      ...
      try {
        ...
        emit(AuthError('La contraseña o password no coinciden'));
    ...
    on<SignUpRequested>((event, emit) async {
      emit(Loading());
      try {
        await authRepository.signUp(email: event.email, password: event.password, displayName: event.name);
        emit(Authenticated());
      } catch (e) {
    ...
    on<DeleteAuth>((event, emit) async {
      emit(Loading());
      await authRepository.deleteAuth();
      emit(UnAuthenticated());
    });
  }
}
